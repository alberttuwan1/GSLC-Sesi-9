...
public class Teacher extends Person implements IsPassChangeable, IsUseId{
	...
	public void viewAllScore() {
		for (Assignment i : assignment) {
			...
			for (Student k : student) {
				...
				for (int x = 0; x < len; x++) {
					if(k.getAssignment().get(x).getAssignmentId().equals(i.getAssignmentId())) {
						System.out.println("Score       : " + k.getAssignmentscore().get(x));
						break;
					}
				}
			}
	...
	public void createAssignment() throws CloneNotSupportedException {
		Assignment asg = new Assignment(this);
		assignment.add(asg);
		// add the assignment to the student's assignment list
		// using cloning
		for (Student i : student) {
			i.getAssignment().add((Assignment)asg.clone());
			i.getAssignmentscore().add(-1);
		}
		System.out.println("The Assignment Has Been Successfully Created"
				+ "\nAnd Given to Your Student");
		Util.enter();
	}
